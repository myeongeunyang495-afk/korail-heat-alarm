...
const REGIONS = [
  { name: "서울", label: "서울(영등포)", nx: 58, ny: 126, primary: true, basis: "영등포" },
  { name: "수원", label: "경기(수원)", nx: 60, ny: 121, primary: true },
  { name: "인천", label: "인천", nx: 55, ny: 124, primary: true }
];
...
async function fetchCurrent(region, now) {
  ...
  if (!Number.isFinite(byCategory.T1H) || !Number.isFinite(byCategory.REH)) {
    throw new Error(`${region.label} 현재 실황 T1H/REH가 없습니다.`);
  }
  return {
    source: "초단기실황",
    date: ymd(base),
    ...
    apparentC: apparentTemperatureC(byCategory.T1H, byCategory.REH)
  };
}
...
async function fetchForecast(region, now) {
  ...
  if (!points.length) throw new Error(`${region.label} 오늘 단기예보 TMP/REH가 없습니다.`);
  return points;
}
...
async function buildRegion(region, now) {
  ...
  return { ...region, current, forecast, ...heatWindow(forecast) };
}
